...
function on_mousemove(ev) {
	...
	var pos = GetPos(ev); //JSU WAS: var pos = {x: e.clientX, y: e.clientY};
...
function GetPos(ev) {
	...
	if (mouse) {
		posx = ev.pageX-beginPos.x;
		posy = ev.pageY-beginPos.y;
	} else {
		posx = ev.originalEvent.targetTouches[0].pageX - beginPos.x; 
		posy = ev.originalEvent.targetTouches[0].pageY - beginPos.y;
	}
...
function on_mouseup() {
	...
	if (path != null) {
		...
		if (refItem) {
			...
			console.info ("Letter found %s", refItem.symbol);
...
function initHw(idCanvas) {	
		...
		canvas.bind('touchstart', function(e) {
			...
			console.log("beginPos " + beginPos.x +" "+ beginPos.y); 
